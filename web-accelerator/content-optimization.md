...
This has resulted in an 8% reduction in file size.


#### Responsive Image Filtering

Responsive image markup that uses the `srcset` attribute with `img` or `picture` and `source` elements can also be optimized to reduce markup size. URLs within `srcset` are resolved so that any that don't apply to the current client are removed. 
